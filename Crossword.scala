...
object Crossword {
  def main(args: Array[String]) {
    ...
    for (i <- 0 to 9) {
      for (j <- 0 to 8) {
        if ((j == 0 || matrix(i)(j - 1) == '+') && matrix(i)(j) == '-' && matrix(
              i)(j + 1) == '-') {
          var c = j
    ...
    for (j <- 0 to 9) {
      for (i <- 0 to 8) {
        if ((i == 0 || matrix(i - 1)(j) == '+') && matrix(i)(j) == '-' && matrix(
              i + 1)(j) == '-') {
          var r = i
  ...
  def solveCrossword(matrix: Array[Array[Char]],
                     words: Buffer[String],
                     hpos: ListBuffer[(Int, Int, Int)],
                     vpos: ListBuffer[(Int, Int, Int)]) {
    ...
    for (wpos <- 0 until words.length) {
      ...
      for (pos <- 0 until hpos.length) {
        ...
        if (word.length == len) {
          var fits = true
          for (j <- 0 until len)
            if (matrix(r)(c + j) != '-' && word(j) != matrix(r)(c + j))
              fits = false
          if (fits) {
            // Save original word
      ...
      for (pos <- 0 until vpos.length) {
        ...
        if (word.length == len) {
          var fits = true
          for (i <- 0 until len)
            if (matrix(r + i)(c) != '-' && word(i) != matrix(r + i)(c))
              fits = false
          if (fits) {
            // Save original word
            val ostr = new StringBuffer
